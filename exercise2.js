...
// 3. Create a subclass `Dog` that overrides the `speak()` method to return "The dog barks", but still use the parent `getType` getter in the subclass.
// 4. Create another subclass `Cat` that overrides the `speak()` method to return "The cat meows", but still use the parent `getType` getter in the subclass.
